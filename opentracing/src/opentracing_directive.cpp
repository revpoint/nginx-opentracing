...
//------------------------------------------------------------------------------
// propagate_uwsgi_opentracing_context
//------------------------------------------------------------------------------
char *propagate_uwsgi_opentracing_context(ngx_conf_t *cf, ngx_command_t *command,
                                         void *conf) noexcept try {
  auto main_conf = static_cast<opentracing_main_conf_t *>(
      ngx_http_conf_get_module_main_conf(cf, ngx_http_opentracing_module));
  if (!main_conf->tracer_library.data) {
    ngx_log_error(NGX_LOG_ERR, cf->log, 0,
                  "opentracing_uwsgi_propagate_context before tracer loaded");
    return static_cast<char *>(NGX_CONF_ERROR);
  }
  if (main_conf->span_context_keys == nullptr) {
    return static_cast<char *>(NGX_CONF_OK);
  ...
  ngx_log_error(NGX_LOG_ERR, cf->log, 0,
                "opentracing_uwsgi_propagate_context failed: %s", e.what());
  return static_cast<char *>(NGX_CONF_ERROR);
}
